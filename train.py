...
from sft_trainer import ExtractionSFTTrainer

sys.path.append('..')
import utils
from generation_dataset import ExtractionDataset, ExtractionEvalDataset
...
def train(args):
    ...
    train_dataset = ExtractionDataset(args.instruction_train_data_path, args.context_train_data_path,
                                         tokenizer=tokenizer, head_inject_rate=args.head_inject_rate, tail_inject_rate=args.tail_inject_rate)
    eval_dataset = ExtractionEvalDataset(args.eval_data_path, tokenizer=tokenizer)
    ...
    eval_dataloader = strategy.setup_dataloader(eval_dataset, batch_size=1,
                                                             shuffle=False, collate_fn=eval_dataset.collate_fn)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--head_inject_rate", type=float, default=0.25)
    parser.add_argument("--tail_inject_rate", type=float, default=0.25)
